...
class NeuralNetwork:
    ...
    def __init__(self, features_num, max_epoch=1000, train_rate=0.05, accuracy=1e-5, regularization_rate=0.0, batch_size=None):
        self.layers_num = 0
    ...
    def train(self, x, y):
        ...
        pre_cost = 9999.0
        m_total = x.shape[1]  # Number of samples

        if self.batch_size is None or self.batch_size > m_total:
        ...
        for epoch in range(self.max_epoch+1):
            ...
            if math.fabs(self.cost-pre_cost) < self.accuracy:
            ...
            while batch_from < m_total:
                xbatch = x[:, batch_from:batch_to]
                ybatch = y[:, batch_from:batch_to]

                m = xbatch.shape[1]      # Number of samples in batch
...
network = NeuralNetwork(features_num=4, max_epoch=20000, train_rate=0.3, accuracy=1e-7, regularization_rate=5e-4, batch_size=None)
network.add_layer(neurons_num=8)
