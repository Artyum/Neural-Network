...
class NeuralNetwork:
    ...
    def __init__(self, features_num, max_epoch=1000, train_rate=0.05, accuracy=1e-5, regularization_rate=0.0, batch_size=0, dropout=0.0):
        self.layers_num = 0
    ...
    def train(self, x_train, y_train, x_test, y_test):
        ...
        for epoch in range(self.max_epoch + 1):
            ...
            if epoch % 1 == 0:
                ...
                print('Epoch: {0}\t|\tCost: {1}\t|\tScore: {2}\t({3})'.format(epoch, self.cost, score, (1000 - end)))

                # Check if training is finished
                if math.fabs(pre_cost - self.cost) < self.accuracy or end == 1000:
                    print('Epoch: {0}  |  Cost: {1}'.format(epoch, self.cost))
                    break
                pre_cost = self.cost

            # Dropout
            if self.dropout > 0.0 and epoch % 100 == 0:
                for i in range(self.layers_num):
        ...
        print('Total time:', time.time() - _time_total)
...
network = NeuralNetwork(features_num=16, max_epoch=9999999, train_rate=1.2, accuracy=1e-10, regularization_rate=0, batch_size=10, dropout=0)
